...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <form action="{{ route('employee-types.update', $type->id) }}" method="post" role="form">
                {{ csrf_field() }}
                {{ method_field('put') }}
                    ...
                    <div class="portlet-body form">
                        <div class="form-body">
                            <div class="form-group form-md-line-input form-md-floating-label">
                                <input type="text" class="form-control" id="name" name="name" value="{{ $type->name or old('name') }}" required>
                            ...
                            <div class="form-group form-md-line-input form-md-floating-label">
                                <select class="form-control" id="payment_structure_id" name="payment_structure_id">
                                    @foreach($structures as $structure)
                                        <option value="{{ $structure->id }}" {{ $type->payment_structure_id or old('payment_structure_id') == $structure->id ? 'selected' : '' }}>{{ $structure->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group form-md-line-input form-md-floating-label">
                                <textarea class="form-control" id="description" name="description" required rows="5">{{ $type->description or old('description') }}</textarea>
