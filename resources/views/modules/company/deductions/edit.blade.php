...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <form action="{{ route('deductions.update', ['deduction' => $deduction->id]) }}" method="post" role="form">
                {{ csrf_field() }}
                {{ method_field('put') }}
                    ...
                    <div class="portlet-body form">
                        <div class="form-body">
                            ...
                            @if($deduction->id != 1 && $deduction->id != 2 && $deduction->id != 3)
                                <input type="text" class="form-control" id="name" name="name" value="{{ $deduction->name }}" required>
                            @else
                                <input type="text" class="form-control" id="name" name="name" value="{{ $deduction->name }}" required readonly>
                            @endif
                            ...
                            <div class="form-group form-md-line-input form-md-floating-label">
                                ...
                                @for($i = 1; $i < 32; $i++)
                                    <option value="{{ $i }}"{{ $i == $deduction->due_date ? ' selected' : '' }}>{{ $i }}</option>
                                @endfor
                                </select>
                                <label for="due_date">Date Due*</label>
                                <span class="help-block">This is the date that the payment is due every month.</span>
                            </div>
                            <div class="form-group form-md-line-input form-md-floating-label">
                                <input type="text" class="form-control" id="threshold" name="threshold" value="{{ $deduction->threshold }}" required>
                                <label for="threshold">Exemption*</label>
                                <span class="help-block">This is the minimum amount that an individual must earn in order to be deducted</span>
                            </div>
                            <div class="form-group form-md-line-input form-md-floating-label">
                                <select name="type" id="type" class="form-control" >
                                    <option value="slab" {{ $deduction->type == 'slab' ? 'selected' : '' }}>Deduction Slab</option>
                                    <option value="per_employee" {{ $deduction->type == 'per_employee' ? 'selected' : '' }}>Per Employee</option>
                                    <option value="rate" {{ $deduction->type == 'rate' ? 'selected' : '' }}>Rate</option>
                                </select>
                            ...
                            <div id="rate_holder" hidden>
                                <div class="form-group form-md-line-input form-md-floating-label input-group">
                                    <input type="text" class="form-control" id="rate" name="rate" value="{{ $deduction->rate }}">
                                ...
                                <div class="form-group form-md-line-input form-md-floating-label row">
                                    ...
                                    <table class="table table-responsive">
                                        ...
                                        @if($deduction->type == 'slab')
                                            <input type="text" name="total_rows" id="total_rows" hidden value="{{ count($slabs) }}">
                                            <?php $i = 1; ?>
                                            @foreach($slabs as $slab)
                                                <tr id="row{{ $i }}">
                                                    <input type="text" name="row_row{{ $i }}" id="row_row{{ $i }}" hidden value="{{ $i }}">
                                                    <td class="row_number">{{ $i }}</td>
                                                    <td><input type="text" name="min_amount{{ $i }}" id="min_amount{{ $i }}" class="form-control" placeholder="0" value="{{ $slab->min_amount }}"></td>
                                                    <td><input type="text" name="max_amount{{ $i }}" id="max_amount{{ $i }}" class="form-control" value="{{ $slab->max_amount == 0 ? '' : $slab->max_amount }}"></td>
                                                    <td><div class="input-group"><input type="text" name="rate{{ $i }}" id="rate{{ $i }}" class="form-control" placeholder="10" value="{{ $slab->rate }}"></div></td>
                            ...
                            <div class="form-group form-md-line-input form-md-floating-label">
                                <select class="form-control" id="has_relief" name="has_relief">
                                    <option value="1" {{ $deduction->has_relief == 0 ?: 'selected' }}>Yes</option>
                                    <option value="0" {{ $deduction->has_relief == 1 ?: 'selected' }}>NO</option>
                                </select>
                            ...
                            <div id="relief_holder" hidden>
                                <div class="form-group form-md-line-input form-md-floating-label">
                                    <input type="text" class="form-control" id="relief_name" name="relief_name" value="{{ $relief->name }}">
                                ...
                                <div class="form-group form-md-line-input form-md-floating-label">
                                    <input type="text" pattern="[0-9]+$" title="Numbers only" class="form-control" id="relief_amount" name="relief_amount" value="{{ $relief->amount }}">
