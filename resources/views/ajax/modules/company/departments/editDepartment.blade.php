    ...
    <div class="row">
        <div class="col-sm-12">
            <form action="{{ route('departments.update', $department->id) }}" method="post" role="form">
                {{ csrf_field() }}
                {{ method_field('put') }}
                        ...
                        <div class="form-group form-md-line-input form-md-floating-label">
                            <select name="branch_id" id="branch_id" class="form-control select2" >
                                @foreach($branches as $branch)
                                    <option value="{{ $branch->id }}" {{ $department->branch_id == $branch->id ? 'selected' : ''}}>{{ $branch->branch_name }}</option>
                                @endforeach
                            </select>
